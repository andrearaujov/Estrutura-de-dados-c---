...
void listadup::insereNoFim(acaoPrograma acao)
{
    ...
    if (vazia() or acao.identificador > ultimo->acao.identificador)
    {
    ...
    {
        noh* atual = ultimo;
        while(atual->anterior != NULL and atual->anterior->acao.identificador > acao.identificador){
...
{
    noh *novo = new noh(acao);
    if (vazia() or acao.identificador < primeiro->acao.identificador)
    {
    ...
    {
        noh* atual = primeiro;
        while(atual->proximo != NULL and atual->proximo->acao.identificador < acao.identificador){
            atual = atual->proximo;
        }
